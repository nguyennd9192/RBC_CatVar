...
try:
    ...
    from weight_score import weight_score
    from ini_sampling import rand_gidx_sampling
    from lasso_in_cluster import revise_KMEANs_by_LASSO
    from confusion_matrix import confusion_matrix
    from normalize import get_Xnorm
...
def spark_cal(data_df, element, target_variable, config, index_job):
    ...
    # # normalize data_df
    X_norm = get_Xnorm(X_matrix=data_df[this_pv].values)
    data_df[this_pv] = X_norm
    ...
    # # save out gidx to file
    return_dict["gidx"] = np.array(group_predict)
